...
use crate::domain::shift::entity::Model as ShiftModel;
use crate::domain::shift::repository::ShiftRepository;
pub use model::Column;
pub use model::Entity;

pub struct SeaOrmShiftRepository {
    db: DatabaseConnection,
}

impl SeaOrmShiftRepository {
...
#[async_trait]
impl ShiftRepository for SeaOrmShiftRepository {
    async fn find_by_id(&self, id: Uuid) -> AppResult<ShiftModel> {
    ...
    async fn create(&self, shift: ShiftModel) -> AppResult<ShiftModel> {
        let active_model: model::ActiveModel = shift.into_active_model();
        let saved = active_model.insert(&self.db).await?;
        Ok(saved)
    }

    async fn find_all(&self, query: PaginationQuery) -> AppResult<PaginatedResponse<ShiftModel>> {
    ...
    async fn update(&self, shift: ShiftModel) -> AppResult<ShiftModel> {
        let active_model: model::ActiveModel = shift.into_active_model();
