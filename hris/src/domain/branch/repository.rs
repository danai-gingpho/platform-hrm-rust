...
use uuid::Uuid;
use crate::domain::branch::entity::Model;
...
pub trait BranchRepository: Send + Sync {
    async fn create(&self, ctx: &TenantContext, branch: Model) -> AppResult<Model>;
    async fn delete(&self, ctx: &TenantContext, id: Uuid) -> AppResult<()>;
    async fn update(&self, ctx: &TenantContext, branch: Model) -> AppResult<Model>;
